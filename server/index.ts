...
(async () => {
  ...
  function startServer(initialPort: number, maxRetries = 5) {
    const tryListen = (p: number, remaining: number) => {
      ...
      httpServer.once("error", (err: any) => {
        if (err && err.code === "EADDRINUSE") {
          ...
          if (remaining > 0) {
            const next = p + 1;
            console.log(`Trying port ${next} (${remaining} retries left)...`);
            setTimeout(() => tryListen(next, remaining - 1), 200);
          } else {
            console.error(`All retries exhausted. Could not bind to a free port.`);
            process.exit(1);
          }
        } else {
